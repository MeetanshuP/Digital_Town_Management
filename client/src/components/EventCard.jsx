...
const EventCard = ({ event }) => {
    ...
    return (
        <div
            className={`bg-white rounded-3xl p-6 shadow-sm border hover:shadow-lg transition-all flex flex-col gap-6 ${event.status === 'completed'
                ...
                <div className="flex-grow">
                    ...
                    <div className="flex justify-between items-start mb-2 gap-2">
                        <div className="flex gap-2">
                            {event.category && (
                                <span
                                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${getCategoryStyles(event.category)}`}
                                >
                                    {event.category}
                                </span>
                            )}
                            ...
                            {event.status && (
                                <span
                                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${getStatusStyles(event.status)}`}
                                >
                                    {event.status}
                                </span>
                            )}
                    ...
                    <div className="flex justify-between items-start gap-4 mb-2">
                        ...
                        <div className="flex items-center gap-1.5 text-sm font-semibold text-purple-700 bg-purple-50 px-3 py-1 rounded-lg shrink-0">
                            <Clock size={14} />
                            {event.eventTime
                                ? new Date(`2000-01-01T${event.eventTime}`)
                                    .toLocaleTimeString([], {
                                        hour: 'numeric',
                                        minute: '2-digit'
                                    })
                    ...
                    <div className="mb-4">
                        <p
                            className={`text-gray-500 text-base transition-all duration-300 ${expanded ? '' : 'line-clamp-2'
                                }`}
                        >
                            {event.description}
                        </p>
